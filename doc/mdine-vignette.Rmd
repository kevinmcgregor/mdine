...
Next, we'll run **mdine**.  Note that additional parameters can be passed to this function, which will be passed on to *rstan::sampling*.  This allows the user to change the parameters in the Hamiltonian Markov Chain process used by **rstan** (e.g. adaptation parameters).  For more information on the possible parameters, please refer to the **rstan** documentation.
```{r, cache=TRUE}
# Running mdine
...
## Diagnostic tools
The object returned by the **mdine** command includes the object returned from **rstan**.  This can be used to perform model diagnostics.
